...
# 2013-09-16
# Adapted by AB Chase on 2017-03-27
...
# to run - makeblobplot.R [input file] 0.01 taxlevel_[tax level] [1 or 0]
# INPUT FILE = the output generated from gc_cov_annotate.pl
# 0.01 is the threshold - if fewer than this proportion of annotated contigs
# taxlevel_* as designated by the user
# [1 or 0] to plot all the coverages profiles [1] or just one plot with the total coverage across all samples [0]
...
# if arg_plotall == 0 we will ignore the sample coverage by each and only graph the total coverage
if (arg_plotall == 0) orig <- orig[, -grep("\\.bam$", colnames(orig))]
